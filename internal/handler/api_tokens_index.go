...
func APITokensIndex(appService *app.Service) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		pageHTML := view.ApiTokensIndex(appService.Styler.Theme(), appService.Localizer, appService.Settings.Language(), csrfToken, presentedTokens)
		htmlString := view.Layout(appService.Styler.Theme(), appService.Localizer, appService.Settings.Language(), "archmark | tokens", pageHTML)
